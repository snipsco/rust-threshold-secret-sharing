...
pub fn fft2(a_coef: &[i64], omega: i64, prime: i64) -> Vec<i64> {
    let zp = ::fields::montgomery::ZprimeField32::new(prime as u32);
    let mut data = a_coef.iter()
        .map(|&value| if value < 0 { value + prime } else { value })
        .map(|a| zp.from(a as u64))
        .collect::<Vec<_>>();
    ::fields::ffts::fft2(&zp, &mut *data, zp.from(omega as u64));
    data.iter().map(|a| zp.back(*a) as i64).collect()
}

/// Inverse FFT for `fft2`.
pub fn fft2_inverse(a_point: &[i64], omega: i64, prime: i64) -> Vec<i64> {
    let zp = ::fields::montgomery::ZprimeField32::new(prime as u32);
    let mut data = a_point.iter()
        .map(|&value| if value < 0 { value + prime } else { value })
        .map(|a| zp.from(a as u64))
        .collect::<Vec<_>>();
    ::fields::ffts::fft2(&zp, &mut *data, zp.inv(zp.from(omega as u64)));
    let scale = zp.inv(zp.from(data.len() as u64));
    data.iter().map(|a| zp.back(zp.mul(*a, scale)) as i64).collect()
}
...
pub fn fft3(a_coef: &[i64], omega: i64, prime: i64) -> Vec<i64> {
    ...
    let mut data = a_coef.iter()
        ...
        .map(|a| zp.from(a as u64))
        .collect::<Vec<_>>();
    ::fields::ffts::fft3(&zp, &mut *data, zp.from(omega as u64));
    data.iter().map(|a| zp.back(*a) as i64).collect()
}
...
pub fn fft3_inverse(a_point: &[i64], omega: i64, prime: i64) -> Vec<i64> {
    ...
    let mut data = a_point.iter()
        ...
        .map(|a| zp.from(a as u64))
        .collect::<Vec<_>>();
    ::fields::ffts::fft3(&zp, &mut *data, zp.inv(zp.from(omega as u64)));
    let scale = zp.inv(zp.from(data.len() as u64));
    data.iter().map(|a| zp.back(zp.mul(*a, scale)) as i64).collect()
}
